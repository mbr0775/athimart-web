// lib/products/admin-product-edit-service.ts
...
/**
 * Load every editable field for one product.
 *
 * The authenticated Supabase client and existing
 * admin RLS policies protect this query.
 */
export async function getAdminEditableProduct(
  productId: string
): Promise<
  AdminEditableProduct | null
> {
  ...
  if (error) {
    throw new Error(
      `Unable to load editable product: ${error.message}`
    );
  }
